...
const Events = function Events(props) {
  ...
  async function add(eventName, startDate, endDate) {
    try {
      await axios.post('/api/admin/events/add', {
        eventName: eventName,
        startDate: startDate,
        endDate: endDate,
      });
  ...
  const events = eventsList.map((event) => (
    <tr key={event._id}>
      <td>{event.eventName}</td> <td>{event.startDate} </td>{' '}
      <td> {event.endDate} </td>{' '}
  ...
  return (
    <div>
      <form onSubmit={submitHandle}>
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => setStartDateToAdd(e.target.value)}
          ></input>
        </label>
        <br></br>
        <label>
          ...
          <input
            type="text"
            value={endDateToAdd}
            onChange={(e) => setEndDateToAdd(e.target.value)}
          ></input>
        </label>
      ...
      <table>
        <tr>
          {' '}
          <th> Event Name</th> <th> Start Date</th> <th> End Date</th>{' '}
        </tr>
        {events}
      </table>
      )
